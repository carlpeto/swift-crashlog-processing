...
struct SwiftSymbolicate: AsyncParsableCommand {
    static var configuration: CommandConfiguration {
        ...
        abstract: "Symbolicate input files containing crash logs and write to output files. Works on streams/pipes as well except on Windows.",
        discussion: """
    ...
    @Option(
        ...
        help: "Additional path(s) to search for/cache symbol files (no effect on machO crash log symbolication).\n(can use environment variable SWIFT_SYMBOLICATE_SYMBOL_PATHS instead)")
    ...
    @Option(
        ...
        help: "Additional path(s) to search for/cache symbol files.\n(can use environment variable SWIFT_SYMBOLICATE_SYMBOL_PATHS instead)")
    var symbolAdditionalPaths: [String] = []    
    #endif
    ...
    #else
    @Option(name: [.short, .long], help: "Where to send the symbolicated output (defaults to stdout)")
    var outputFile: String?
    #endif
    ...
    @Option(
        name: [.long],
        help: "Debuginfod symbol server(s) (baseURL), tried in order\n(or use environment variable SWIFT_SYMBOLICATE_GDB_SERVERS instead)")
    var gdbSymbolServers: [String] = []

    @Option(
        name: [.long],
        help: "Windows symbol server(s) (baseURL), tried in order. Only uncompressed PDB files are supported; CAB-compressed (.pd_) and file.ptr redirect files are not handled.\n(or use environment variable SWIFT_SYMBOLICATE_WINDOWS_SERVERS instead)")
    var windowsSymbolServers: [String] = []

    func validate() throws {
        let env = ProcessInfo.processInfo.environment
        let hasGdbServers = !gdbSymbolServers.isEmpty
            || env["SWIFT_SYMBOLICATE_GDB_SERVERS"] != nil
        let hasWindowsServers = !windowsSymbolServers.isEmpty
            || env["SWIFT_SYMBOLICATE_WINDOWS_SERVERS"] != nil
        let hasSymbolPaths = !symbolAdditionalPaths.isEmpty
    ...
    static func backtraceOpts(swiftBacktraceEnvSettings: String?) ->
        [Substring:Substring] {

        guard let swiftBacktraceEnvSettings
        else { return [:] }

        return swiftBacktraceEnvSettings
            .split(separator: ",")
    ...
    static func getJsonBacktraceFormatterOptions(
        swiftBacktraceEnvSettings: String?) -> BacktraceJSONFormatterOptions {
    ...
    static func plainCrashLogFormattingOptions(opts sb_opts: [Substring:Substring]) ->
        ...
        var options: BacktraceFormattingOptions = switch sb_opts["preset"] {
        case "full":
    ...
    mutating func run() async throws {
        ...
        let btOptions = Self.backtraceOpts(swiftBacktraceEnvSettings:
        ...
        let cacheUpdatePolicy: CacheUpdatePolicy = switch env["SWIFT_SYMBOLICATE_CACHE_UPDATE"]?.lowercased() {
            case "always": .always
            case "newer": .newer
            default: .never
        }

        let allSymbolPaths = symbolAdditionalPaths
            + (env["SWIFT_SYMBOLICATE_SYMBOL_PATHS"]?
                .split(separator: ";")
                .map(String.init) ?? [])

        let gdbServerURLs = gdbSymbolServers
            + (env["SWIFT_SYMBOLICATE_GDB_SERVERS"]?
                .split(separator: ";")
                .map(String.init) ?? [])

        let windowsServerURLs = windowsSymbolServers
