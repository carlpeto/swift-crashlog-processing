...
public struct Recognizer {
    ...
    public enum SignaturePart {
        ...
        func matches(
            byte: UInt8,
            skipCount: inout Int) -> (Bool, NextAction) {
    ...
    public mutating func scanByte(byte: UInt8) -> RecognitionStatus {
        print_dr("scanning 0x\(String(byte, radix: 16)) against " +
        "signature position \(recognizedPositionInSignature) " +
        "of \(signature.count)")
...
public extension Array where Element == Recognizer.SignaturePart {
    ...
    init(_ string: String) {
        self.init(bytes: Array<UInt8>(string.utf8))
    }
    ...
    init?(skipTo: Character, max: Int) {
        guard let skip = Array<UInt8>(skipTo.utf8).first else { return nil }
